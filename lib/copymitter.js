...
const copyFile = zames(require('@cloudcmd/copy-file'));
...
Copymitter.prototype.cpOneFile = function(from, to) {
    ...
    const emitRetry = () => emitter.emit('retry');
    
    const noDirectory = (e) => e && e.code === 'ENOENT' && e.path === dirname(to);
    ...
    stat(dirname(to))
        .then(wraptile(copyFile, from, to, [countStream]))
        .then(emitFile)
        .catch((e) => {
            if (noDirectory(e))
                return copyDir(dirname(from), dirname(to))
                    .then(emitRetry)
                    .catch(emitError)
