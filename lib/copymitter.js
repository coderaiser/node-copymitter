...
const emitDir = wraptile((emitter, from, to) => emitter.emit('directory', from, to));
const cleanReturn = () => {};

const emitCount = wraptile((emitter, i, n) => emitter.emit('count', i, n));
const emitSize = (emitter, i, n) => emitter.emit('size', i, n);
...
Copymitter.prototype._cpAll = function() {
    ...
    cpEmitter.on('end', () => {
        ...
        this.emit('file', fromFull, toFull);
        this.emit('count', this._countI, this._countN);
...
Copymitter.prototype._copyDir = function(from, to, fn) {
    stat(from)
        .then(getMode)
        .then(mkdir(to))
        .then(emitDir(this, from, to))
        .then(emitCount(this, from, to))
        .then(cleanReturn)
        .then(fn)
        .catch(fn);
}
...
Copymitter.prototype.cpOneFile = function(from, to) {
    ...
    const countStream = through2((chunk, encoding, cb) => {
        ...
        emitSize(this, this._i, this._size);
        this._progress();
