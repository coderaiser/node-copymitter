...
const {read, write} = require('redzip');
...
async function copy(from, to, {emitError, emitFile, emitDirectory, countStream}) {
    const [statError, info] = await tryToCatch(stat, from);
    
    if (statError)
        return emitError(statError);
...
async function superCopy(from, to, [countStream]) {
    await copyDir(dirname(from), dirname(to));
